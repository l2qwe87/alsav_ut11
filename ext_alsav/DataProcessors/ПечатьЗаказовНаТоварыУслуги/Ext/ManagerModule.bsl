...
Процедура алсав_Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода)
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЗаказПоставщикуСклад") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"ЗаказПоставщикуСклад",
			НСтр("ru = 'Заказ поставщику (склад)'"),
			СформироватьПечатнуюФормуЗаказаПоставщикуСклад(СтруктураТипов, ОбъектыПечати, ПараметрыПечати));
...
Функция СформироватьПечатнуюФормуЗаказаПоставщикуСклад(СтруктураТипов, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	ТабличныйДокумент.ИмяПараметровПечати	= "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказПоставщикуСклад";
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		ЗаполнитьТабличныйДокументЗаказаНаТоварыУслугиСклад(
			ТабличныйДокумент,
			ДанныеДляПечати,
			ОбъектыПечати,
			"Обработка.ПечатьЗаказовНаТоварыУслуги.ПФ_MXL_ЗаказПоставщикуСклад");
...
Процедура ЗаполнитьТабличныйДокументЗаказаНаТоварыУслугиСклад(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ИмяМакета)
...
#Область ОпределениеИменЗаголовковОбластей
		...
		ЕстьДопПараметр    = Ложь;
		ДвойнойДопПараметр = Ложь;
		
		ПостфиксКолонок = "Склад";
...
#Область ОбластиМакета
		...
		ОбластьНомераНДС		= Макет.ПолучитьОбласть("ПодвалТаблицыСклад|НомерСтроки");
		ОбластьКодовНДС			= Макет.ПолучитьОбласть("ПодвалТаблицыСклад|КолонкаКодов");
		ОбластьТоварНДС			= Макет.ПолучитьОбласть("ПодвалТаблицыСклад|Товар"  + ПостфиксКолонок);
		ОбластьДанныхНДС		= Макет.ПолучитьОбласть("ПодвалТаблицыСклад|Данные" + ПостфиксКолонок);
...
Процедура алсав_ЗаполнитьТабличныйДокументЗаказаНаТоварыУслуги(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ИмяМакета)
	ШаблонОшибкиТовары	= НСтр("ru = 'В документе %1 отсутствуют товары. Печать %2 не требуется'");
	ШаблонОшибкиЭтапы	= НСтр("ru = 'В документе %1 отсутствуют этапы оплаты. Печать %2 не требуется'");
...
#Область ВыводТаблицыТовары
		...
		Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
			...
			///АЛСАВ +++ номенклатура партнеера
			УстановитьПараметр(ОбластьТоварСтроки,"АртикулИсполнителя",СтрокаТовары.АртикулИсполнителя);
			УстановитьПараметр(ОбластьТоварСтроки,"НаименованиеНоменклатурыИсполнителя",СтрокаТовары.НаименованиеНоменклатурыИсполнителя);
			///---
