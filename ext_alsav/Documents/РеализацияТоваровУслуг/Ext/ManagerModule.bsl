...
Функция УстановитьЗначениеФактическиОтгружен(Ссылка, Значение) ЭКСПОРТ

	Если ТипЗнч(Ссылка) <> Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		Возврат 0;
	КонецЕсли;

	Если НЕ Пользователи.РолиДоступны("АЛСАВ_ФактическиОтгружен") Тогда
		Возврат 0;
	КонецЕсли;
	
	//Сообщить(Ссылка);      
	
	заголовокСвойства = "Фактически отгружен";
	
	текущееЗначение = алсав_ДопРеквизиты.ПолучитьДопРеквизит(Ссылка, заголовокСвойства);
	
	Если текущееЗначение <> Значение Тогда                                                  
		об = Ссылка.ПолучитьОбъект();
		алсав_ДопРеквизиты.УстановитьЗначениеДопРеквизита(об, заголовокСвойства, Значение);	
